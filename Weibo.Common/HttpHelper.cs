...
namespace Weibo.Common
{
    ...
    public static class HttpHelper1
    {
        ...
        /// <returns></returns>
        public static string GetHttpsHtml(string url, string refer, ref string realtaobaolink)
        {
            var uri = new Uri(url);
            ...
            realtaobaolink = res.ResponseUri.ToString();
        ...
        /// 获取字符串中两个指定字符之间的所有字符
        /// </summary>
        /// <param name="source">需要处理的字符</param>
        /// <param name="output">输出</param>
        /// <param name="sstr">开始字符</param>
        /// <param name="estr">结束字符</param>
        /// <param name="index">取字符时，字符开始的索引</param>
        /// <returns>是否成功</returns>
        public static bool GetStringInTwoKeyword(string source, ref string output, string sstr, string estr, int index)
        {
            try
            {
                if (source.Equals(""))
                ...
                int num = source.IndexOf(sstr, index);
                int num2 = -1;
                int num3 = -1;
                num3 = source.IndexOf(estr, (int)(num + sstr.Length));
                if (num3 != -1)
                {
                    num2 = num3;
                }
                ...
                output = source.Substring(num + sstr.Length, num2 - (num + sstr.Length));
                return true;
            }
            catch
            {
        ...
        public static bool GetStringInTwoKeyword(string source, ref string output, string sstr, string estr, int index, ref int endIndex)
        {
            try
            {
                if (source.Equals(""))
        ...
        public static string GetBdShortUrl(string url)
        {
            ...
            while (shorturlresult.Contains("-1"))
            {
                ...
                JSONObject shorturljson = JSONConvert.DeserializeObject(shorturlresult);
                if (shorturljson.Count == 1) continue;
                bdshorturl = shorturljson["tinyurl"].ToString();
            }
            return bdshorturl;
        ...
        public static string SendDataByGET(string Url, string refer, ref CookieContainer cookie)
        {
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(Url);
            if (cookie.Count == 0)
            {
        ...
        /// Http下载文件
        /// </summary>
        public static string HttpDownloadFile(string url, string path, CookieContainer cc)
        {
            try
            {
                // 设置参数
                ...
                response.Headers["Content-Disposition"] = "attachment;filename=" + HttpUtility.UrlEncode(path, System.Text.Encoding.UTF8);
